...
const ListPost = () => {
  ...
  const handleToggleLike = async (accountId: string, postId: string) => {
    ...
    const message = isAlreadyLiked
      ? `Bạn đã bỏ thích bài viết của ${accountToUpdate.username}`
      : `Bạn đã thích bài viết của ${accountToUpdate.username}`;
  ...
  const handleCommentSubmit = async (accountId: string, postId: string) => {
    ...
    addNotification(
      `Bạn đã bình luận bài viết của ${
        account.find((acc) => acc.id === accountId)?.username
      }: "${newComment.text}"`
    );
    try {
      await dispatch(
              ...
              <Card.Body className="explore-card-body">
                <Row>
                  ...
                  <Col xs={3}>
                    <i
                      className={`bi ${
                        isAccountFollowed(account.id) ? "bi-check" : "bi-plus"
                      } explore-card-icon`}
                      onClick={() => handleIconClick(account.id)}
                      style={{
                        cursor: "pointer",
                        fontSize: "60px",
                        color: isAccountFollowed(account.id)
                          ? "#8EE86A"
                          : "#FF8B8B",
                      }}
                    />
              ...
              >
                <div className="card-detail row">
                  ...
                  >
                    ...
                    >
                      <Button
                        onClick={() => handleFollowClick(account.id)}
                        className={
                          isAnimatingBtn[account.id] ? "button-animate" : ""
                        }
                        style={{
                          backgroundColor: isAccountFollowed(account.id)
                            ? "#8EE86A"
                            : "#FF8B8B",
                          color: "#fff",
                          border: "none",
                      ...
                      >
                        {isAccountFollowed(account.id) ? "Following" : "Follow"}
                      </Button>
                      ;
